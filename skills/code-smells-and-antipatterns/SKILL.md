...
This skill is the diagnostic library. Each smell has a code example, the cost it imposes, and a pointer to the named refactoring(s) that fix it. For the *application* of those refactorings — when to refactor, how to do it safely, the discipline of small steps — see the sibling skill `refactoring-and-code-smells`.
...
- `refactoring-and-code-smells` — the corrective side. Each smell here maps to one or more named refactorings there.
