...
- **Hoare, "The Emperor's Old Clothes"** (Turing Award lecture, 1980). The "billion-dollar mistake" of null references.
...
- **Martin Kleppmann, *Designing Data-Intensive Applications*** (2017, 2nd ed. 2024). The current standard reference for data systems and distributed-systems trade-offs. Reliability, scalability, maintainability; consistency models; consensus.
...
- **Sandi Metz, *Practical Object-Oriented Design in Ruby*** (2012). The "Sandi Metz rules" (≤100 line classes, ≤5 line methods, ≤4 parameters), and a usable take on OO that doesn't dogmatize.
...
- **Fallacies of Distributed Computing** (Deutsch & Gosling, Sun) — the network is reliable, latency is zero, bandwidth is infinite, the network is secure, topology doesn't change, there is one administrator, transport cost is zero, the network is homogeneous. All false.
